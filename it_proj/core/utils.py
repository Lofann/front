...
def get_vacancies():
    try:
        params = {
            'text': 'team lead', #Тут название твоей професии
            'specialization': 1,
            'page': 1,
            'per_page': 100,
            'date_from': f'{datetime.datetime.now()}', #дат за которую собирать
            'date_to': f'2022-12-13T00:00:00+0300',
        }
        data = []
        info = get('https://api.hh.ru/vacancies', params).json()
        ...
        for vacancy in data[len(data) - 10:]:
            vacancies.append(clean_vacancy(get(f'https://api.hh.ru/vacancies/{vacancy["id"]}').json()))
        return vacancies
